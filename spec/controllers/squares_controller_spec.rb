...
RSpec.describe SquaresController do
  ...
  describe 'GET index' do
    ...
    it 'sets @squares' do
      ...
      expect(assigns(:squares)).to eq(%w[A B C])
      expect(response).to be_successful
    end
  end
  ...
  describe 'POST create' do
    context 'when creating a new square' do
      context 'and save is successful' do
        it 'creates the square, sets success flash, and redirects to area_squares_path' do
          allow(db).to receive(:save_doc).with({ 'temp-doc' => true, 'square' => 'D',
        ...
        it 'uppercases the square name' do
          allow(db).to receive(:save_doc).with({ 'temp-doc' => true, 'square' => 'Z',
                                                 'area' => area_id }).and_return(true)
    ...
    context 'when square already exists' do
      ...
      it 'does not attempt to save the document' do
        expect(db).not_to receive(:save_doc) # rubocop:disable RSpec/MessageSpies
